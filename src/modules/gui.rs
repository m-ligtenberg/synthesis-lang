...
pub fn window(args: &[Value]) -> crate::Result<Value> {
    if args.is_empty() {
        return Err(anyhow::anyhow!("window requires a title argument"));
    }
    
    let title = match &args[0] {
        Value::String(s) => s.clone(),
        _ => return Err(anyhow::anyhow!("window title must be a string")),
    };
...
pub fn button(args: &[Value]) -> crate::Result<Value> {
    if args.is_empty() {
        return Err(anyhow::anyhow!("button requires a label argument"));
    }
    
    let label = match &args[0] {
        Value::String(s) => s.clone(),
        _ => return Err(anyhow::anyhow!("button label must be a string")),
    };
...
pub fn slider(args: &[Value]) -> crate::Result<Value> {
    if args.len() < 3 {
        return Err(anyhow::anyhow!("slider requires label, min, max arguments"));
    }
    
    let label = match &args[0] {
        Value::String(s) => s.clone(),
        _ => return Err(anyhow::anyhow!("slider label must be a string")),
    };
    
    let min_val = args[1].as_number()
        .ok_or_else(|| anyhow::anyhow!("slider min must be a number"))?;
    let max_val = args[2].as_number()
        .ok_or_else(|| anyhow::anyhow!("slider max must be a number"))?;
    
    if min_val >= max_val {
        return Err(anyhow::anyhow!("slider min must be less than max"));
    }
...
pub fn checkbox(args: &[Value]) -> crate::Result<Value> {
    if args.is_empty() {
        return Err(anyhow::anyhow!("checkbox requires a label argument"));
    }
    
    let label = match &args[0] {
        Value::String(s) => s.clone(),
        _ => return Err(anyhow::anyhow!("checkbox label must be a string")),
    };
...
pub fn dropdown(args: &[Value]) -> crate::Result<Value> {
    if args.len() < 2 {
        return Err(anyhow::anyhow!("dropdown requires label and options arguments"));
    }
    
    let label = match &args[0] {
        Value::String(s) => s.clone(),
        _ => return Err(anyhow::anyhow!("dropdown label must be a string")),
    };
        ...
        Value::Array(arr) => {
            ...
            for opt in arr {
                match opt {
                    ...
                    _ => return Err(anyhow::anyhow!("dropdown options must be strings")),
                }
            }
            opts
        }
        _ => return Err(anyhow::anyhow!("dropdown options must be an array")),
    };
    
    if options.is_empty() {
        return Err(anyhow::anyhow!("dropdown must have at least one option"));
    }
...
pub fn control_group(args: &[Value]) -> crate::Result<Value> {
    if args.is_empty() {
        return Err(anyhow::anyhow!("control_group requires a title argument"));
    }
    
    let title = match &args[0] {
        Value::String(s) => s.clone(),
        _ => return Err(anyhow::anyhow!("control_group title must be a string")),
    };
    ...
    controls.insert("title".to_string(), Value::String(title));
